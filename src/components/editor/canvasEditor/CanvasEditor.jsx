...
export default function CanvasEditor({image, id, activeId, canvasSize, canvasExportSize}) {
  ...
  return (
    <div className={`${activeId === id ? 'active' : ''} visibleImage`}>
        <TextEditor id={id}/>

        <Stage width={canvasSize.width} height={canvasSize.height}  style={{maxWidth: "100%", overflow: "auto", backgroundColor: "white"}}>
            <Layer>
                {image && (
                    <KonvaImage
                        // ref={imageRef}
